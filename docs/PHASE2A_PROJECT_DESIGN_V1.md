...
| `SOURCE_UNREACHABLE` | 凭据过期 / 网络**临时**不可达 | 来源验证 | **transient（有界重试）** | 重试期内留在 RECEIVED/SOURCES_OK + 重试标记（Project 实体已存在）；**超限 → 升级为 `REJECTED`（原因码仍记 `SOURCE_UNREACHABLE`，持久化在 Project 表）** |
